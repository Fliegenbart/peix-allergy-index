...
function buildRegionalTrendsUrl(trendsGeo) {
  return `https://trends.google.com/trending/rss?geo=${trendsGeo}`;
}
...
async function buildLiveSnapshot({
  ...
  const regionalDemandResults = await fetchRegionalGoogleTrends(
    REGIONS,
    targetDate,
    fetchImpl
  );
  if (regionalDemandResults.demand.length > 0) {
    demand = regionalDemandResults.demand;
    sourceStatus.push({
      source: "GOOGLE_TRENDS_REGIONAL",
      status: regionalDemandResults.liveCount >= 8 ? "live" : "partial",
      regionCount: regionalDemandResults.liveCount,
      fallbackCount: regionalDemandResults.fallbackCount,
      message: `Regional trends for ${regionalDemandResults.liveCount}/${REGIONS.length} states`,
    });
  } else {
    try {
...
async function fetchRegionalGoogleTrends(regions, targetDate, fetchImpl) {
  const results = await Promise.allSettled(
    regions.map(async (region) => {
      const url = buildRegionalTrendsUrl(region.trendsGeo);
      const xml = await fetchText(url, 3000, fetchImpl);
      const items = parseGoogleTrendsRss(xml);
      const scored = scoreGoogleDemand(items);
      return {
        regionCode: region.code,
        regionName: region.name,
        targetDate,
        searchInterest: scored.score,
        searchDelta: scored.delta,
        matchedTopics: scored.matchedTopics,
        matchedTraffic: scored.matchedTraffic,
        source: "GOOGLE_TRENDS_REGIONAL",
      };
    })
  );
  ...
  return { demand, liveCount, fallbackCount };
}
...
module.exports = {
  ...
  buildLiveSnapshot,
  buildRegionalTrendsUrl,
  fetchAirQuality,
  fetchRegionalGoogleTrends,
  fetchWeather,
  resolveTargetDate,
};
