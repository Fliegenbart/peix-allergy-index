...
function buildActivationRationale({
  regionName,
  regionCode,
  mediaIndex,
  action,
  ...
  topPollen,
  rainBrake,
  suggestedBudgetShiftPct,
  drivers,
}) {
  const parts = [];
  const actionLabels = {
    boost: "BOOST (+150% Budget)",
    prepare: "PREPARE (+60% Budget)",
    hold: "HOLD (0% Änderung)",
    pause: "PAUSE (-100% Budget)",
  };

  parts.push(`Region: ${regionName} (${regionCode})`);
  parts.push(`Empfehlung: ${actionLabels[action] || action.toUpperCase()}`);
  parts.push(`Media Index: ${mediaIndex}/100 | Confidence: ${confidence}%`);
  parts.push("");
  parts.push("BEGRÜNDUNG:");

  if (rainBrake) {
    parts.push(
      "• Regen-Bremse aktiv: Niederschlag unterdrückt Pollenflug und reduziert Allergiesymptome."
    );
  }

  if (topPollen && topPollen.index03 > 0) {
    const pollenLevel =
      topPollen.index03 >= 2.5
        ? "sehr hoch"
        : topPollen.index03 >= 1.5
          ? "hoch"
          : topPollen.index03 >= 0.5
            ? "moderat"
            : "niedrig";
    parts.push(
      `• Pollenbelastung (${topPollen.type}): ${pollenLevel} (DWD Index ${topPollen.index03}/3)`
    );
  } else {
    parts.push("• Keine signifikante Pollenbelastung gemeldet.");
  }

  if (componentScores.weather >= 70) {
    parts.push("• Wetterbedingungen begünstigen Pollenflug (trocken, windig, warm).");
  } else if (componentScores.weather <= 30) {
    parts.push("• Wetterbedingungen hemmen Pollenflug.");
  }

  if (componentScores.airQuality >= 70) {
    parts.push("• Luftqualität kann Atemwegsreizung verstärken (erhöhte PM/Ozon-Werte).");
  }

  if (componentScores.demand >= 70) {
    parts.push("• Erhöhtes regionales Suchinteresse für Allergie-Begriffe beobachtet.");
  } else if (componentScores.demand <= 30) {
    parts.push("• Geringes regionales Suchinteresse für Allergie-Begriffe.");
  }

  parts.push("");
  parts.push("KONTEXT:");
  parts.push(
    `• Signal-Quellen: Pollen ${componentScores.pollen}, Wetter ${componentScores.weather}, Luft ${componentScores.airQuality}, Demand ${componentScores.demand}`
  );
  parts.push(`• Budget-Shift-Empfehlung: ${suggestedBudgetShiftPct >= 0 ? "+" : ""}${suggestedBudgetShiftPct}%`);

  if (drivers && drivers.length > 0) {
    parts.push(`• Haupt-Treiber: ${drivers.map((d) => d.replace(/_/g, " ")).join(", ")}`);
  }

  parts.push("");
  parts.push("COMPLIANCE:");
  parts.push("• Targeting-Modus: Kontextuell/Umwelt-basiert (keine persönlichen Gesundheitsdaten)");
  parts.push("• Datenquellen: DWD Pollenflug, Open-Meteo Wetter/Luft, Google Trends Regional");

  return parts.join("\n");
}

function buildTransparencyReport(snapshot) {
  const generatedAt = new Date().toISOString();
  const regions = snapshot.rankings.map((region) => ({
    regionCode: region.regionCode,
    regionName: region.regionName,
    action: region.action,
    mediaIndex: region.mediaIndex,
    suggestedBudgetShiftPct: region.suggestedBudgetShiftPct,
    rationale: buildActivationRationale(region),
  }));

  const summary = {
    boostRegions: regions.filter((r) => r.action === "boost").map((r) => r.regionCode),
    prepareRegions: regions.filter((r) => r.action === "prepare").map((r) => r.regionCode),
    holdRegions: regions.filter((r) => r.action === "hold").map((r) => r.regionCode),
    pauseRegions: regions.filter((r) => r.action === "pause").map((r) => r.regionCode),
  };

  const totalBudgetImpact = regions.reduce((sum, r) => {
    const weight = 1 / regions.length;
    return sum + r.suggestedBudgetShiftPct * weight;
  }, 0);

  return {
    reportType: "PEIX Allergy Media Transparency Report",
    generatedAt,
    targetDate: snapshot.targetDate,
    summary: {
      ...summary,
      totalRegions: regions.length,
      averageBudgetShiftPct: round(totalBudgetImpact, 1),
    },
    methodology: {
      version: snapshot.methodology.version,
      validationStatus: snapshot.methodology.validationStatus,
      note: "Gewichtungen sind MVP-Priors. Backtest gegen Outcome-Daten erforderlich vor Automatisierung.",
    },
    privacyStatement: {
      mode: snapshot.privacyMode,
      description:
        "Dieses Tool verwendet ausschließlich öffentliche Umweltdaten (Pollen, Wetter, Luftqualität) und aggregierte Suchdaten. Es werden keine individuellen Gesundheitsprofile erstellt oder verwendet.",
      dataRetention: "Snapshots werden für Validierungszwecke gespeichert, enthalten aber keine personenbezogenen Daten.",
    },
    regions,
    auditTrail: {
